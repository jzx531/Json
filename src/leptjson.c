...
static void lept_parse_whitespace(lept_context *c) {
    // 直接通过指针修改 c->json
    while (*c->json ==' ' || *c->json == '\t' || *c->json == '\r' || *c->json == '\n')
        c->json++; 
}*/
...
static int lept_parse_string(lept_context *c,lept_value *v)
{
    ...
    for(;;)
    {
        ...
        switch(ch){
            case '\"':
                //字符串结束符"\"",停止解析,并把字符串内容复制到v->u.s.s中
                len = c->top - head;
...
static void lept_stringify_value(lept_context* c, const lept_value* v) {
    ...
    switch (v->type) {
        ...
        /*%.17g 的含义：
g：自动选择 %f（小数）或 %e（科学计数法）中较短的一种格式。
17：保留 17 位有效数字。这是为了保证 double 类型的精度不丢失（IEEE 754 双精度浮点数大约需要 17 位十进制数来保证往返转换不损失精度）。*/
...
void lept_copy(lept_value* dst, const lept_value* src) {
    ...
    switch (src->type) {
        case LEPT_NULL:
            ...
            break;
        case LEPT_FALSE:
            lept_set_boolean(dst, 0);
            break;
        case LEPT_TRUE:
            lept_set_boolean(dst, 1);
            break;
        case LEPT_NUMBER:
            lept_set_number(dst, src->u.n);
            break;
        case LEPT_STRING:
...
int lept_is_equal(const lept_value* lhs, const lept_value* rhs) {
    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            {
                notfind = 1;
                for(int j = 0; j < rhs->u.o.size; j++)
                {
                    if(strcmp(lhs->u.o.m[i].k, rhs->u.o.m[j].k) == 0)
                    {
                        notfind = 0;
...
// test02中只含number时使用v->n，其他时候使用v->u.n
